    ...
    private fun downloadZoneAroundMe() {
        ...
        Thread {
            try {
                ...
                val allTiles = mutableListOf<Long>()
                for (zoom in DOWNLOAD_MIN_ZOOM..DOWNLOAD_MAX_ZOOM) {
                    val xMin = lonToTileX(bbox.lonWest, zoom)
                    val xMax = lonToTileX(bbox.lonEast, zoom)
                    val yMin = latToTileY(bbox.latNorth, zoom)
                    val yMax = latToTileY(bbox.latSouth, zoom)
                    for (x in xMin..xMax) for (y in yMin..yMax)
                        allTiles.add(MapTileIndex.getTileIndex(zoom, x, y))
                }
                val total = allTiles.size
                ...
                handler.post { tvDownloadStatus.text = "❌ Erreur — vérifie ta connexion"; btnDownloadZone.isEnabled = true }
            }
        }.start()
    }
